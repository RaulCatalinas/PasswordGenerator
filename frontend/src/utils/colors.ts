...
export function getNotificationColors({ darkModeActive, type }: GetNotificationColorsProps): RGBColor {
  const mode = darkModeActive ? "DarkMode" : "LightMode"
  const notificationType = type.toUpperCase() as keyof typeof NOTIFICATION_COLORS.DarkMode
